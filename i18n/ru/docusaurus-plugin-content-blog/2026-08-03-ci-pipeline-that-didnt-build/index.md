...
import deskImage from '@site/blog/2026-08-03-ci-pipeline-that-didnt-build/1785748277.webp';
import villageImage from '@site/blog/2026-08-03-ci-pipeline-that-didnt-build/1785748363.webp';
...
- Docker-образ для Sega Mega Drive не содержал git. Checkout молча переключился на REST API. Сабмодули сломались. Никто не закричал.
...
<details>
<summary>Почему checkout молчит</summary>

`actions/checkout` проверяет наличие git в PATH. Если git не найден — не падает. Не предупреждает. Меняет стратегию без единого слова. Метод с tarball'ом через REST API быстрее и не требует git. Но ломается на `submodules: recursive`. Что и произошло.
...
**«`cancel-in-progress: false`. Новый push отменит активный деплой.»** Технически верно. Но сборки с веток должны гаснуть, когда ветка уходит вперёд, а деплой на GitHub Pages защищён отдельной группой на уровне джобы. С `false` устаревшие сборки встали бы в очередь.
...
Самое неприятное — как эти нарушения проявляются. Ни одно не ломает CI громко. Checkout молча переключается на REST API. CMake печатает «Manually-specified variables were not used by the project» и идёт дальше. Неиспользуемые permissions не вызывают ошибок. Тихие баги. Ждут своего часа.
...
<img src={deskImage} alt="Рабочий стол детектива у окна. Блокнот с таблицей из четырёх зелёных галочек — Linux, macOS, Windows, Sega MD. Лупа лежит на распечатке YAML-файла. Чашка чая с паром. Книжный шкаф и кружевные занавески на фоне." />
...
<img src={villageImage} alt="Миниатюрная английская деревня. Четыре домика, соединённых трубами: с флюгером-пингвином, с яблоней, с витражным окном и в форме Sega Genesis. Пожилой детектив в твиде осматривает протекающий стык с блокнотом в руке. Живая изгородь и чайные розы в тёплом дневном свете." />
...
Есть одна вещь, которая меня беспокоит. Когда CI падает молча — как checkout без git, который не роняет сборку, а меняет стратегию, — это я понимаю. Страшновато, но хотя бы воспроизводимо.
